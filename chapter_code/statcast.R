...
## ----statcastr_hide, eval=FALSE, echo=TRUE-------------------------------
## library(statcastr)
...
## ----prob_hr, warning=FALSE, fig.cap="Contour graph of the fitted probability of a home run as a function of exit velocity and launch angle using a generalized additive model using 2016 season data."----
...
## ----prob_hr5, warning=FALSE, fig.cap="Predictive distribution of the number of 2017 home runs using a generalized additive model on the 2016 season data.  The actual number of home runs in 2017 is shown as a vertical line."----
ggplot(HR_predict, aes(HR)) +
  geom_histogram(color = "black", fill = "white", bins = 20) +
  geom_vline(xintercept = pull(actual_hr), color = crcblue) +
  annotate(geom = "text", x = 6000, y = 200,
...
## ----launch_angles, fig.cap="Distribution of launch angles across batted balls for regular batters in 2017."----
...
## ----launch_angles_hl, fig.cap="Distribution of launch angles across batted balls for regular batters in 2017. Matt Carpenter had the highest average launch angle, while Dee Gordon was the only player with an average launch angle close to 0. Domingo Santana has the smallest variance in launch angle."----
library(ggrepel)
la_plot + 
...
## ----la_cor_plot, fig.cap="Scatterplot of launch angle across batters for the first and second halves of the 2017 season. Launch angle shows strong correlation among batters."----
